...
void Channel::broadcastMessage(const std::string& message, Client* sender) {
	for (std::map<int, Client*>::iterator it = _members.begin(); it != _members.end(); it++) {
		...
		if (send(it->first, message.c_str(), message.length(), 0) == -1)
			errorln("Error sending message to " << it->second->get_username() << ".");
	}
}
...
bool Channel::addMember(Client* client) {
	...
	if (_members.find(client->get_fd()) != _members.end()) {
		errorln(client->get_username() << " is already a member of '" << _name << "'!");
	}
	else {
...
int Channel::checkChannelModes(char mode) const {
	switch (tolower(mode)) {
		case 'i':
			return _inviteOnly;
		case 't':
